...
BUILD_COMMIT_MESSAGE := $(shell git log -1 --pretty=%B HEAD)
PROFILE := $(or $(PROFILE), local)
ENVIRONMENT := $(shell ([ $(PROFILE) = local ] && echo local) || ( echo $(BUILD_BRANCH) | grep -Eoq '^task/[A-Z]{2,5}-[0-9]{1,5}_[A-Za-z0-9_]{4,32}' && (echo $(BUILD_BRANCH) | grep -Eo '^task/[A-Z]{2,5}-[0-9]{1,5}' | grep -Eo '[A-Z]{2,5}-[0-9]{1,5}' | tr '[:upper:]' '[:lower:]') || ([ $(BUILD_BRANCH) = master ] && echo $(PROFILE) || echo unknown)))
USER_ID := $(shell id -u)
GROUP_ID := $(shell id -g)
